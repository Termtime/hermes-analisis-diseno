...
public class MantUsuariosPage implements Initializable {
    ...
    private void btnModificarUsuarioClick(ActionEvent event) {
        if (tablaUSelectedItem != null) {
            Navigation.pushRouteWithParameter("MantUsuariosModalModificarUsuario", event, false, true,
                    MantUsuariosModalModificarUsuario.class, tablaUSelectedItem);
            llenarTabla();
        } else {
    ...
    @FXML
    private void txtFiltroInput(KeyEvent event) {
        tablaUsuarios.getItems().clear();
        List<User> usuariosFiltrados = new ArrayList<User>();
        for (User usuario : usuarios) {
            if (usuario.nombre.toLowerCase().contains(txtFiltro.getText().toLowerCase())
                    ...
                    public void run() {
                        ...
                        Platform.runLater(new Runnable() {
                            ...
                            public void run() {
                                ...
                                double realWidth = Math.min(imagenUsuario.getFitWidth(),
                                        imagenUsuario.getFitHeight() * aspectRatio);
                                double realHeight = Math.min(imagenUsuario.getFitHeight(),
                                        imagenUsuario.getFitWidth() / aspectRatio);
    ...
    private void llenarTabla() {
        ...
        for (DocumentSnapshot doc : documentos) {
            ...
            if (doc.exists()) {
                ...
                tmp = new User(doc.getId(), doc.getString("Nombre"), doc.getString("nivelAcceso"), doc.getString("uid"),
                        areasConNombre, areasConID, doc.getBoolean("deshabilitado"));
                usuarios.add(tmp);
            }
        }
